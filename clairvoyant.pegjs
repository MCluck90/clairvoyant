/**
 * This defines the language used by Clairvoyant to produce a Psykick skeleton
 *
...
PropertyAssignment
    = name:PropertyName _ "=>" __ value:Literal {
        return {
...
SubBlockList
    = head:SubBlock tail:(__ SubBlock)* {
        var result = [head];
        for (var i = 0, len = tail.length; i < len; i++) {
...
ComponentList
    = head:Component __ tail:(__ Component)* {
        var result = [head];
        for (var i = 0, len = tail.length; i < len; i++) {
            result.push(tail[i][1]);
        }
...
Component
    = name:Identifier __ value:ObjectLiteral {
        return {
            type: "Component",
            name: name,
